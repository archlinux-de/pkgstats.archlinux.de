...
func NewSQLiteRepository(db *sql.DB) *SQLiteRepository {
	return &SQLiteRepository{
		db:           db,
		samplesCache: database.NewMonthlySamplesCache(db, `SELECT month, SUM(count) FROM operating_system_architecture GROUP BY month`),
	}
}
...
func (r *SQLiteRepository) FindByName(ctx context.Context, name string, startMonth, endMonth int) (*OperatingSystemArchitecturePopularity, error) {
	var count int
	query := `SELECT COALESCE(SUM(count), 0) FROM operating_system_architecture WHERE name = ? AND month >= ? AND month <= ?`

	if err := r.db.QueryRowContext(ctx, query, name, startMonth, endMonth).Scan(&count); err != nil {
		return nil, fmt.Errorf("query OS architecture count: %w", err)
	}
...
func (r *SQLiteRepository) FindAll(ctx context.Context, query string, startMonth, endMonth, limit, offset int) (*OperatingSystemArchitecturePopularityList, error) {
	samples, err := r.getSamples(ctx, startMonth, endMonth)
	if err != nil {
		return nil, fmt.Errorf("get samples: %w", err)
	}
	...
	sqlQuery = `
		...
		FROM operating_system_architecture
	...
	countQuery = `SELECT COUNT(DISTINCT name) FROM operating_system_architecture WHERE month >= ? AND month <= ?`
	if query != "" {
	...
	if err := r.db.QueryRowContext(ctx, countQuery, countArgs...).Scan(&total); err != nil {
		return nil, fmt.Errorf("count OS architectures: %w", err)
	}

	rows, err := r.db.QueryContext(ctx, sqlQuery, args...)
	if err != nil {
		return nil, fmt.Errorf("query OS architectures: %w", err)
	}
	defer func() { _ = rows.Close() }()

	var archs []OperatingSystemArchitecturePopularity
	for rows.Next() {
		var name string
		var count int
		if err := rows.Scan(&name, &count); err != nil {
			return nil, fmt.Errorf("scan OS architecture: %w", err)
		}

		archs = append(archs, OperatingSystemArchitecturePopularity{
			Name:       name,
			Samples:    samples,
			Count:      count,
			Popularity: calculatePopularity(count, samples),
	...
	if err := rows.Err(); err != nil {
		return nil, fmt.Errorf("iterate OS architectures: %w", err)
	}

	if archs == nil {
		archs = []OperatingSystemArchitecturePopularity{}
	}

	return &OperatingSystemArchitecturePopularityList{
		Total:                                   total,
		Count:                                   len(archs),
		OperatingSystemArchitecturePopularities: archs,
		Limit:                                   limit,
		Offset:                                  offset,
		Query:                                   &query,
	}, nil
...
func (r *SQLiteRepository) FindSeriesByName(ctx context.Context, name string, startMonth, endMonth, limit, offset int) (*OperatingSystemArchitecturePopularityList, error) {
	countQuery := `SELECT COUNT(*) FROM operating_system_architecture WHERE name = ? AND month >= ? AND month <= ?`
	var total int
